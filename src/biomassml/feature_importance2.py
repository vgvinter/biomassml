...
def get_shap_values(model, X, x_scaler, y_scaler,
...
def custom_wrapper_CH4(model, X, y_scaler):
    ...
    y_scaler: y scaler for TARGETS_GASIF = CO, H2, COMB, GAS
    """
    y_pred_mu_CO = predict_coregionalized(model, X, 0)[0]*sqrt(y_scaler.var_[0]) + y_scaler.mean_[0]
    y_pred_mu_H2 = predict_coregionalized(model, X, 1)[0]*sqrt(y_scaler.var_[1]) + y_scaler.mean_[1]
    y_pred_mu_COMB = predict_coregionalized(model, X, 2)[0]*sqrt(y_scaler.var_[2]) + y_scaler.mean_[2]
...
class GPYEstimator:
    ...
    def predict(self, X):
        ...
        y_pred_mu = predict_coregionalized(
            self.model, X, self.i)[0]*sqrt(self.y_scaler.var_[self.i]) + self.y_scaler.mean_[self.i]
...
class GPY_CH4_Estimator:
    ...
    def predict(self, X):
        X = scale_X(X, self.x_scaler)
        y_pred_mu_CO = predict_coregionalized(self.model, X, 0)[0]*sqrt(self.y_scaler.var_[0]) + self.y_scaler.mean_[0]
        y_pred_mu_H2 = predict_coregionalized(self.model, X, 1)[0]*sqrt(self.y_scaler.var_[1]) + self.y_scaler.mean_[1]
        y_pred_mu_COMB = predict_coregionalized(self.model, X, 2)[0]*sqrt(self.y_scaler.var_[2]) + self.y_scaler.mean_[2]
        y_pred_mu_CH4 = y_pred_mu_COMB - y_pred_mu_CO - y_pred_mu_H2
        return y_pred_mu_CH4.flatten()
