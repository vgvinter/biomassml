...
def predict_H2(X, model, x_scaler, y_scaler):
    ...
    X_scaled = x_scaler.transform(X)
    y_pred_mu_H2 = (
        predict_coregionalized(model, X_scaled, 1)[0] * sqrt(y_scaler.var_[1]) + y_scaler.mean_[1]
    )
...
def predict_COMB(X, model, x_scaler, y_scaler):
    ...
    X_scaled = x_scaler.transform(X)
    y_pred_mu_COMB = (
        predict_coregionalized(model, X_scaled, 2)[0] * sqrt(y_scaler.var_[2]) + y_scaler.mean_[2]
    )
...
def predict_GAS(X, model, x_scaler, y_scaler):
    ...
    X_scaled = x_scaler.transform(X)
    y_pred_mu_GAS = (
        predict_coregionalized(model, X_scaled, 3)[0] * sqrt(y_scaler.var_[3]) + y_scaler.mean_[3]
    )
...
def predict_CH4_covar(X, model, x_scaler, y_scaler, DATA_DIR):
    ...
    cov1 = (
        np.corrcoef(y["volCOMB (%)"], y["volCO (%)"])[0, 1]
        * y_pred_std_COMB.flatten()
        * y_pred_std_CO.flatten()
    )
    cov2 = (
        np.corrcoef(y["volCOMB (%)"], y["volH2 (%)"])[0, 1]
        * y_pred_std_COMB.flatten()
        * y_pred_std_H2.flatten()
    )
    cov3 = (
        np.corrcoef(y["volCO (%)"], y["volH2 (%)"])[0, 1]
        * y_pred_std_CO.flatten()
        * y_pred_std_H2.flatten()
    )
    y_pred_std_CH4_covar = sqrt(
        np.sum(np.square([y_pred_std_COMB, y_pred_std_CO, y_pred_std_H2]), axis=0).flatten()
        - 2 * cov1
        - 2 * cov2
        + 2 * cov3
    )
...
def predict_HHV(X, model, x_scaler, y_scaler):
    ...
    y_pred_std_HHV = additive_errorprop(
        [11.76 / 100 * y_pred_std_CO, 11.882 / 100 * y_pred_std_H2, 37.024 / 100 * y_pred_std_CH4]
    )
...
def predict_H2CO(X, model, x_scaler, y_scaler):
    ...
    y_pred_std_H2CO = (
        additive_errorprop([y_pred_std_H2 / y_pred_mu_H2, y_pred_std_CO / y_pred_mu_CO])
        * y_pred_mu_H2CO
    )
...
def predict_Eyield(X, model, x_scaler, y_scaler):
    ...
    y_pred_std_Eyield = (
        additive_errorprop([y_pred_std_HHV / y_pred_mu_HHV, y_pred_std_GAS / y_pred_mu_GAS])
        * y_pred_mu_Eyield
    )
